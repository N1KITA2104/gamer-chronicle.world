...
$query = "SELECT posts.*, IFNULL(SUM(post_rating_value), 0) AS total_rating, category.category_name 
          ...
          WHERE visible = 1 AND category.category_name = 'Ігри'
    ...
    <?php while ($row = mysqli_fetch_assoc($result)): ?>
        ...
        <div class="news-item text-light">
            ...
            <div class="news-caption description">
                <a href="index.php?action=view_post&id=<?= $row['post_id'] ?>"><?= strip_tags($row['post_title']) ?></a>
            </div>

            <?php if (!empty($_SESSION) && $_SESSION['user_access_type'] === 1): ?>
                <div class="service-buttons">
                    <button class="edit-button" title="Редагувати пост" onclick="window.location.href = 'index.php?action=edit_post&id=<?= $row['post_id'] ?>'">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button class="edit-button" title="Видалити пост" onclick="window.location.href = 'index.php?action=confirm_delete_post&id=<?= $row['post_id'] ?>'">
