            ...
            <ul class="list-group">
                ...
                foreach ($all_activities as $activity) : ?>
                    <li class="list-group-item">
                        ...
                        // Формируем ссылку на пост в зависимости от типа активности
                        switch ($activity['type']) {
                            case 'post':
                                echo '<b>' . $formatted_date . '</b> | <a class="profile-last-activity" href="index.php?action=ViewPost&id=' . $activity['post_id'] . '">Новий пост: ' . $activity['activity_title'] . '</a>';
                                break;
                            case 'comment':
                                echo '<b>' . $formatted_date . '</b> | <a class="profile-last-activity" href="index.php?action=ViewPost&id=' . $activity['post_id'] . '">Новий коментар: ' . $activity['comment'] . '</a>';
                                break;
                            case 'reaction':
                                $reaction_value = ($activity['reaction'] == 'upvote') ? '1' : '-1';
                                $reaction_class = ($reaction_value == '1') ? 'text-success' : 'text-danger';
                                echo '<b>' . $formatted_date . '</b> | <a class="profile-last-activity ' . $reaction_class . '" href="index.php?action=ViewPost&id=' . $activity['post_id'] . '">Реакція на пост: ' . $reaction_value . '</a>';
                                break;
                        }
                        ?>
                    </li>
