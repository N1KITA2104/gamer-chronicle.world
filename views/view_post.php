...
<main class="container bg-light p-3 pb-3">
    <article>
        ...
        <section class="mb-3 d-flex flex-column align-items-center section-container p-2 pe-3 mt-3">
            <?php if (!empty($_SESSION) && $_SESSION['user_access_type'] === 1): ?>
                <button style="height:32px;width:32px" class="btn btn-danger mb-2 d-flex align-items-center justify-content-center" title="Редагувати пост" onclick="window.location.href = 'index.php?action=edit_post&id=<?php echo $row['post_id']; ?>';"><i class="fas fa-edit"></i></button>
                <button style="height:32px;width:32px"  class="btn btn-danger mb-2 d-flex align-items-center justify-content-center" id="deleteForm" title="Видалити пост" onclick="window.location.href = 'index.php?action=confirm_delete_post&id=<?php echo $row['post_id']; ?>';"><i class="fas fa-trash-alt"></i> </button>
            <?php endif; ?>
    ...
    <?php include("views/comments_block.php"); ?>
</main>

<script>
    function vote(reaction) {
        $.ajax({
            url: 'views/update_rating.php',
            type: 'POST',
            data: {
                id: <?php echo $post_id; ?>,
                vote: reaction
